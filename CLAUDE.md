...
- a **tables** folder (e.g. `src/core/utilityTables/`) — one builder function per file, also walked one subfolder deep, emitted as `<functionName>().forEach(x => this.items.push(x))`.
...
- `typeName` on a row stands in for the class name a repository entry would have, and is one of the things vendors narrow stock by — every row of a table shares one constructor, so without it they are indistinguishable. `Ability`, `Modifier`, `AbilityObject` and `DescriptiveNumber` all default `typeName` to `this.constructor.name` for the repository case.
- **Categories are not row fields.** A table holds one category and tags all of its rows at once through the `categorise` callback: `buildModifiers(ROWS, m => m.modifierType = Modifier.Type.Constraint)`, `buildAttacks(ROWS, a => a.attackType = Attack.AttackType.Spell)`, `buildUtilities(ROWS, u => u.subtype = 'wizard')`. That is what the table files are named after (`constraints.ts`, `spells.ts`, `debuffs.ts`, `wizardAttacks.ts`), and it is the grouping the vendor rule below permits. A `Utility`'s `cooldown` is not one of these — it varies row by row rather than table by table, so it stays a plain row field.
- **`Activity.subtype` is the free-form one**, a plain string rather than an enum, and it exists so a vendor can stock a whole authored table without naming its rows one by one — `abilities: { subtypes: ["wizard"] }`. Not to be confused with `Attack.attackType` (`Weapon` / `Spell`, rendered on the card as "Attack Type"), which is a genuine enum on an axis the generator reads: `buildAttacks` rolls an attack's element from it. Nothing reads `subtype` except `Vendor.compile`.
...
**Abilities are narrowed by vendor, never by character class.** Which content a character can get is decided by the active `Vendor`'s stock (`src/core/vendor.ts`), not by a class affinity on the content itself. Generation must never read a selected class — there is deliberately no `CharacterContext.classes` to read. `weight` stays for genuine rolling odds (rarity, or a component that only makes sense on one kind of ability, e.g. `manaFumeModifier` returning `0` unless the affector is an `Attack`); leave it at the default `1` when the vendor is the only thing that should gate availability. Repository folders are flat and must not be grouped by class — no `wizardAttacks/`, no `wizardModifiers/`. If a repository grows enough to need subfolders, split it on something the generator actually reads (attack type, element, rarity), never on a character class.

**A class vendor's own list is the one exception, and it lives in a tables folder**: `attackTables/classAttacks/` and `utilityTables/classUtilities/` hold one table per class (`wizardAttacks.ts`, `wizardUtilities.ts`), because the rows a class vendor sells are authored together as a spell list and are only ever reached through that vendor's stock. The exception is about where a file sits, not about what generation may read — the rows are ordinary rows, carry no class field, and are tied to the wizard by nothing but a `subtype: 'wizard'` tag that only `vendors.ts` gives a meaning to. Do not let it spread to a repository folder, to the component tables, or to any code path: `subtype` is a stock key, and generation reading it would be exactly the class affinity this rule forbids.

This constrains the **content**, not the vendor: a class-flavoured vendor is free to stock whatever it likes, attacks included, through its `VendorStock`. The owner has explicitly settled this — class vendors **will** sell attacks, and the `Wizard` vendor now does. Nothing may grow a "does this class fight?" notion in response.

**Vendor stock slots** (`VendorStockList` in `src/core/vendor.ts`): `abilities` is shorthand covering both ability factories; `attacks` and `utilities` override it individually; `modifiers`, `abilityObjects`, and `descriptiveNumbers` cover the components. **An omitted slot stocks everything**, so refusing a slot has to be explicit — `utilities: Vendor.NOTHING`, which is how `BASIC_TRAINER` stays an attack trainer now that nothing else narrows the kind a visit teaches. Prefer the narrow slot over `abilities` when a vendor means one kind: `abilities` silently narrows utilities too, which is rarely what a "teaches these attacks" vendor wants. A subtype that spans both kinds is the case `abilities` is for — that is how `WIZARD` stocks its whole list in one line.

Each slot's `VendorStock` matches an item that satisfies **any** of its criteria: `subtypes` against `Activity.subtype`, `types` against `typeName`, `names` against the display name, plus a `where` predicate for anything else. Reach for `subtypes` when a vendor owns a whole table and `types` when it takes single pieces out of a shared pool (`BASIC_TRAINER`, `GRAZED_TRAINER`) — a list of row names in a stock declaration is a sign the content wants a subtype instead.

**`Vendor.NO_ALTERATIONS`** is the vendor's third constructor argument, and it turns off everything the generator would otherwise roll on top of an authored row: an attack's alterations (its modifiers and the flavoured damage number that replaces the flat one) and a utility's extra modifiers from `Utility.MODIFIER_CHANCE`. Both read it through `Vendor.altersAbilities()`, which is `true` outside any vendor. It is mostly for class vendors, whose point is a fixed list taught as written — a `NO_ALTERATIONS` attack has a zero power residual by construction, so `AttackCompensationService` finds nothing to move and no `Compensation` appears on the card. What it does *not* turn off is anything a row asks for itself or anything correctness depends on: a utility still draws the `AbilityObject`s its row names, still divides its chance by their rarity, and still takes a `repeatableModifier` if that leaves the chance above 1.

Empty stock means two different things depending on the factory, controlled by `Factory.canSellNothing()`. The ability factories override it to `true` — a vendor that stocks no attacks sells none. The component factories leave it `false` and fall back to the full pool, because they are built midway through assembling an ability that is already committed to; returning nothing there strands callers like `buildUtilities`, which indexes straight into `get(1)[0]` when drawing a row's components.
...
**The balancing pass:** the heart of the generator. `Utility.compensate()` reconciles a generated utility against a fixed DPS/power budget (`Utils.getDPS`), adjusting chance, value, range coefficients, and even injecting extra modifiers (`Utility.MODIFIER_CHANCE`) or a `repeatableModifier` when power overflows. When editing generation, preserve the budget math here rather than fudging output numbers. Balancing constants live in `src/core/utils.ts` and `src/core/characterContext.ts`.

**Attacks balance against their own row, not the budget** (`AttackCompensationService`, built in `Attack.generate()` *before* alterations run so it can snapshot the row as authored). An attack row is balanced by definition; only what the generator adds on top of it — its alterations, and a flavoured damage number replacing the flat one — has to be paid back, so `Utils.getDPS()` never enters an attack's math and an unaltered row emerges with exactly the numbers its author wrote. Raw power is just `damage × chance`: the row's author already priced its range and its accuracy band in, which is why there is no range or DPS coefficient anywhere in this math and why nothing adjusts an attack between the row and compensation. The service solves for the same power three ways — chance, damage, or mana at `MANA_TO_DAMAGE` (1 mana = 2 damage) — and applies exactly one: the three knobs are shuffled per compensation (`shuffled()`, off `Utils.random()` so it stays seed-reproducible) and the first that lands inside its limits (chance `0.1`–`0.9`, damage `>= 1`, mana `>= 0`, mana also rounded up to an integer) wins; if none fits, the one that gets closest, clamped. No knob is preferred over another — dropping the `shuffled()` call in `compensate()` is what would restore a fixed order. Each knob is solved by walking the power residual to zero rather than by inverting the formula, because a modifier's `powerBonus` may read the very chance or damage being solved for.

Whatever it moved is then recorded as a `Compensation` (`src/modifiers/compensation.ts`) — a `Modifier` with the default no-op power functions, pushed onto the attack so it renders in the card's modifier list, and held on `Attack.compensation` so `getDescription` can mark the one stat it touched. It lives outside `modifiersRepository/` because it is generator output, not stock; `buildFactories` never sees it and it never enters a `WeightedList`. It also implements `StatChange` (`src/core/activity.ts`), the shape `renderCard` reads to colour a stat green or red and hang an up/down arrow off it — green whenever the move favours the player, so mana rising is red while chance rising is green.
...
**Entry point:** `index.ts` attaches `onSeedChange`/`onLevelChange`/`onVendorChange`/`onRulingChange`/`generateAbilities` to `window`. **A visit always teaches `ABILITIES_PER_VISIT` (4) abilities.** That count is fixed and belongs to `index.ts` alone — the vendor decides *which kind* a visit teaches and never *how many*. `teach()` draws them one at a time, each from a factory it builds for that one draw, precisely so the number cannot follow the depth of the stock: an ability is rolled in its constructor, so a vendor holding a single utility teaches four different rolls of it. Do not reintroduce a "draws × abilities per draw" split — `Factory.get` clamps a request to what the vendor stocks, so any draw wider than 1 silently turns a thin stock into a short visit. **The stock is the only thing that decides attack or utility, and the level is not part of that decision** — each draw picks a kind by weighing `AttackFactory.getTotalWeight()` against `UtilityFactory.getTotalWeight()` as the vendor stocks them, so a vendor selling one kind sells it at every level and one selling both sells a mix in proportion to its shelves. The owner has settled this twice: no level parity, no odd/even preference, no "this level wants attacks" anywhere. A vendor refuses a kind by filtering it out (`utilities: Vendor.NOTHING`) and in no other way; one stocking neither renders a "has nothing to teach" note. `getDescription(showRulings)` produces the HTML; the `showRulings` flag appends the `<b>Rulings</b>` long-description block.
...
- Some balancing is deliberately disabled and looks like a bug: `Utils.getDPS` ignores its `level` argument, and `compensationModifier`/`repeatableModifier` are weight-`0` on purpose. The owner's `//TODO` comments at each site record the intended values — don't "fix" the code or drop the comments.
