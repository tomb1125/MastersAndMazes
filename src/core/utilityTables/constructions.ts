...
const ROWS: UtilityRow[] = [
    {
        ...
        description: (wall: Utility) => 'On success create a thin Wall with length up to '+wall.value.getDescription()+' squares and 3 square heigth standing firmly on ground. It can be only climbed only with Climb Movement. As you create it, you can create one turn for each total 10 squares of length, but no sharp turns are allowed. Wall '+wall.objects[0].description
    }
];
